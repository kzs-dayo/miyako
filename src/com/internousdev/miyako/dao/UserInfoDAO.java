...
public class UserInfoDAO {
	...
	public UserInfoDTO getUserInfo(String loginId) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				userInfoDTO.setRegistDate(resultSet.getDate("regist_date"));
				userInfoDTO.setUpdateDate(resultSet.getDate("update_date"));
				if (resultSet.getString("user_id").equals("admin") && resultSet.getString("password").equals("admin")) {
					userInfoDTO.setLoginAdmin(true);
				}
			}
	...
	public int isLogined(String loginId) {
		...
		Connection connection = dbConnector.getConnection();
		int logined = 0;
		String sql = "select logined from user_info where user_id = ?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, loginId);
			...
			if(rs.next()){
				 logined = Integer.parseInt(rs.getString("logined"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
