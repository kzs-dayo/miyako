...
//		 tempUserId = String.valueOf(session.get("tempUserId"));
	 ...
	 boolean duplicationFlg = cartInfoDao.sameProductExistByUser(session.get("loginId").toString(),session.get("tempUserId").toString(),productId);
	 ...
	 if(!duplicationFlg){
		 ...
		 int count = cartInfoDao.updateProductCount(session.get("loginId").toString(),session.get("tempUserId").toString(),productId,Integer.parseInt("productCount"),price);
		 if(count > 0){
			 result = SUCCESS;
		 }
